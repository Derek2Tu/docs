...
## Functionality
The `/integrations/Intercom` and `integrations/connect` endpoints sync articles by default, but tickets and conversations can also be optionally synced.
...
- `auto_synced_source_types`: An array specifying the types of items to sync. Valid values are "ARTICLE" (default), "TICKET", and "CONVERSATION". To sync both articles and tickets, include both values in the array.
...
#### Conversations
Carbon can sync conversations and their associated file attachments. The following ticket information is available as tags for filtering:
