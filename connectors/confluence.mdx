...
We currently don't have a File Selector UI for Confluence (Coming Soon). In the meantime, you can retrieve all Confluence pages for each user using the `list` endpoint and specify specific files to sync via the `sync` endpoint.
...
5. In the "Authorization" tab, add https://api.carbon.ai/integrations/confluence to the `Callback URL`. You can also opt to use a custom CNAME record pointing to https://api.carbon.ai. The domain paths must still be `/integrations/conflence`.
...
You can also use the `resync_file` API endpoint to programmatically resync specific Confluence pages. To delete Confluence pages from Carbon, you can use the `deletefile` endpoint directly.
