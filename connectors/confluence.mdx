...
We currently don't have a File Selector UI for Confluence (Coming Soon). In the meantime, you can retrieve all Confluence pages for each user using the `/integrations/confluence/list` endpoint and specify specific files to sync via the `/integrations/confluence/sync` endpoint.
