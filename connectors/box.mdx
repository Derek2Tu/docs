...
All text, audio and image files are supported file formats via Box.
...
Syncs are triggered when end-users select files to upload via the Box file selector UI. You can also use the `resync_file` API endpoint to programmatically resync specific Box files. To delete Box files from Carbon, you can use the `delete_files` endpoint directly.
