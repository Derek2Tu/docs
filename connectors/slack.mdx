...
- Access tokens will expire every 12 hours and need to be refreshed using the token refresh flow
- White labeling is supported by allowing developers to create their own Slack app.
...
6. Under "OAuth and Permissions" > "Advanced token security via token rotation", click "Opt In". Save the `User OAuth Token` and share the credentials securely with Carbon.
