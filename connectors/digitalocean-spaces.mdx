...
<Note>Since the Spaces API is interoperable with the AWS S3 API, Digital Ocean Spaces leverages our existing S3 endpoints. To determine if a connection belong to DigitalOcean, check `data_source_metadata` equals `DIGITALOCEAN` on the `data_source` object. To determine if a file is synced from DigitalOcean, check `type` equals `DIGITALOCEAN` under the `file_metadata` object.</Note>
