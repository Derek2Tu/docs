...
All text, audio and image files are supported file formats via Dropbox.

<Note>Dropbox has a limit of 50 MB per file, so a single uploaded file cannot exceed 50 MB.</Note>
...
Syncs are triggered when end-users select files to upload via the Dropbox file selector UI. You can also use the `resync_file` API endpoint to programmatically resync specific Dropbox files. To delete Dropbox files from Carbon, you can use the `delete_files` endpoint directly.
