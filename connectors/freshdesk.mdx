...
To sync only tickets or both articles and tickets, use the `file_sync_config` parameter. The `file_sync_config` parameter can also enable syncing attachments from ticket comments.
...
### Article
Carbon can sync `Published` articles under multiple Freshdesk workspace. Please note that `Draft` articles won't be synced.
### Tickets
...
```
{
  ...
  "ticket_submitter": "swapnil@carbon.ai"
}
```
