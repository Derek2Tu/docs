...
### Syncing Options
Use the `file_sync_config` parameter to control what gets synced:
...
- `file_types`: An array specifying the types of items to sync. Valid values are "ARTICLE" (default) and "TICKET". To sync both articles and tickets, include both values in the array.
