...
Set the query parameter `generate_sparse_vectors` to `true` when using the `/uploadfile`, `/upload_text`, and `/upload_file_from_url` endpoints.
...
To enable hybird search, you must set the `hybrid_search` paramemter in the `/embeddings` request body to `TRUE`. 
