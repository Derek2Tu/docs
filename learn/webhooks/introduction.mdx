---
title: "Introduction"
description: "Use webhooks to notify your application about Carbon events."
---

<Card title="Jupyter Notebook" icon="link" href="https://colab.research.google.com/drive/1kXJ29dYOtJQip6NYrx9PKUJDC40PGxU_#scrollTo=2IOAlXG1PGT_">
...
<Tip>You can create a tunnel to your localhost server using a tool like ngrok. For example: https://5f9f-98-14-198-85.ngrok-free.app/api/webhook</Tip>
...
are no spaces between key-value pairs or between keys and values. 
For example,
```
{"x":5,"y":6}
```
...
<Note>Please decode the signing key as a hex string.</Note>
...
Please note that the WebhookType refers to specific events outlined in the [Event Types documentation](event-types). Presently, the `WebhookObjectType` is restricted to `FILE`.
...
<AccordionGroup>
  <Accordion title="What is the retry schedule?">
    If Carbon does not receive a 200 response from a webhook server, we will retry the webhooks once more.
  </Accordion>

  <Accordion title="What happens if the retry fails?">
