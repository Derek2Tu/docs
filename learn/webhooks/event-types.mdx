...
Sent if a organization user has attempts to upload files that would cause them to exceed the organization daily upload limit (2.5GB per day), max file limit, max file per upload limit.
...
#### `REVOKE`
This event is triggered when a user’s access token is revoked via the `/revoke_access_token` endpoint or when Carbon receives a 401 Unauthorized from the data source using an expired or revoked access token.
### Example Payload
