...
| `splitRows`| Boolean | No | If `split_rows` is set to `true`, CSV rows will be automatically split if they exceed either the specified chunk size or the maximum token limit of the embedding model. Can be assigned to the `allowedFileTypes` under `LOCAL_FILES` or 3rd party enabled integrations. Defaults to `false`.
...
<AccordionGroup>
  ...
  `GMAIL`: This integration lets you upload emails from Gmail, along with file attachements. You can reference the supported file formats [here](learn/files/text).
  ...
  `OUTLOOK`: This integration lets you upload emails from Gmail, along with file attachements. You can reference the supported file formats [here](learn/files/text).
