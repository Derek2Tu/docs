...
If you are using Carbon to generate embeddings, in the Carbon Connect component, the specification of an Embedding Model ( `OPENAI`, `AZURE_OPENAI`, `COHERE_MULTILINGUAL_V3`, `VERTEX_MULTIMODAL`) can be set at different levels:
...
<AccordionGroup>
  ...
  `LOCAL_FILES`: This integration lets you upload files from your local machine. You can pass the following configuration for this integration:
    ...
    - `embeddingModel`: Specifies the embedding model for integration. Options include `OPENAI`, `AZURE_OPENAI`, `COHERE_MULTILINGUAL_V3` for text and audio files, and `VERTEX_MULTIMODAL` for image files.
  ...
  <Accordion title="Notion">
    ...
    - `embeddingModel`: Specifies the embedding model used. Options are `OPENAI`, `AZURE_OPENAI`, or `COHERE_MULTILINGUAL_V3`.
    - `generateSparseVectors`: Toggle to `true` to generate sparse vectors for hybrid search. Default is `false`.
    - `prependFilenameToChunks`: Adds the file title to each chunk for the integration. Defaults to `false`. 
  ...
  `FRESHDESK`: This integration lets you upload files from your Freshdesk. You can pass the following configuration for this integration:
    - `chunkSize`: Number of tokens per chunk. Defaults to 1500.
    - `overlapSize`: Size of the overlap in tokens. Defaults to 20.
    - `skipEmbeddingGeneration`: Toggle to skip embedding generation. Defaults to `false`.
    - `embeddingModel`: Specifies the embedding model used. Options are `OPENAI`, `AZURE_OPENAI`, or `COHERE_MULTILINGUAL_V3`.
