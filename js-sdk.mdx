...
description: "A vanilla JavaScript with TypeScript support designed to facilitate seamless integration of the Carbon API into various products or systems."
---

<Warning>Our _*Javascript SDK*_ is being deprecated by March 31, 2024 and replaced by the _*Typescript SDK*_ found [here](/resources/sdks).  </Warning>
<Card title="Github Repo" icon="lightbulb" iconType="duotone" color="#ca8b04" href="https://github.com/hubbleai/sdk-example">
...
### Health
<AccordionGroup>
<Accordion title="getCarbonHealth()">
### getCarbonHealth()
...
<Accordion title="generateAccessToken()">
...
<Accordion title="getWhiteLabelData()">
...
<Accordion title="getUserConnections()">
...
<Accordion title="generateOauthurl()">
...
- **Parameters**: The `generateOauthurl()` method accepts an object with the following properties:
  ...
  - `generateSparseVectors`: Toggle to `true` to generate sparse vectors for hybrid search for a specified integration. Default is `false`.
  - `prependFilenameToChunks`: Adds the file title to each chunk. Defaults to `false`. 
...
<Accordion title="getUserDataSources()">
### getUserDataSources()
...
<Accordion title="revokeAccessToDataSource()">
...
- **Parameters**: The `uploadFiles()` method accepts an object with the following properties:
  ...
  - `chunkOverlap` (number, optional): Defines the chunk overlap. Defaults to 20.
  - `skipEmbeddingGeneration` (boolean, optional): If set to true, embedding generation will be skipped. Defaults to false.
  - `useOcr` (boolean, optional): Toggle to enable Optical Character Recognition (OCR) for PDFs. Default is `false`.
  - `generateSparseVectors` (boolean, optional): Toggle to generate sparse vectors for hybrid search. Default is `false`.
  - `embeddingModel` (type:`EmbeddingModel`, optional): Specifies the embedding model for integration. Options include `OPENAI`, `AZURE_OPENAI`, `COHERE_MULTILINGUAL_V3` for text and audio files, and `VERTEX_MULTIMODAL` for image files.
...
- **Parameters**: The `uploadText()` method requires an object with the following properties:
  ...
  - `embeddingModel` (type:`TextEmbeddingModel`, optional): Specifies the embedding model for integration. Options include `OPENAI`, `AZURE_OPENAI`, `COHERE_MULTILINGUAL_V3` for text and audio files, and `VERTEX_MULTIMODAL` for image files.
...
<Accordion title="deleteFiles()">
### deleteFiles()

- **Description**: This function allows for the removal of multiple files from the Carbon service.
...
- **Returns**: A promise that resolves to an `DeleteFilesResponse` object containing the following properties:

  - `status` (number): The HTTP status code of the response.
  - `data` (object or null): Contains details of the deleted files or any additional response data.
  - `error` (string or null): An error message in case there is an issue with the file deletion.
...
<Accordion title="uploadFileFromUrl()">
### uploadFileFromUrl()
...
### Websites
<AccordionGroup>
<Accordion title="processSitemapUrl()">
### processSitemapUrl()
...
<Accordion title="submitScrapeRequest()">
### submitScrapeRequest()
...
- **Parameters**: The `submitScrapeRequest()` method accepts an object with the following properties:
  ...
  - `enableAutoSync`: Enables scheduled syncs of the submitted URL. Defaults to `false`.
  - `generateSparseVectors`: Toggle to `true` to generate sparse vectors for hybrid search. Default is `false`.
...
<Accordion title="submitSitemapScrapeRequest()">
...
<Accordion title="fetchYoutubeTranscript()">
...
<Accordion title="getRawFilePresignedUrl()">
...
<Accordion title="getParsedFilePresignedUrl()">
